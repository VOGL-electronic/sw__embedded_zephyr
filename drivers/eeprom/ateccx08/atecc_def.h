...
/* size of response packet containing 72 bytes data */
...
/* GenDig zone id config. Use KeyID to specify any of the four 256-bit blocks of the Configuration
 * zone.
 */
#define GENDIG_ZONE_CONFIG       ((uint8_t)0)
/* GenDig zone id OTP. Use KeyID to specify either the first or second 256-bit block of the OTP
 * zone.
 */
#define GENDIG_ZONE_OTP          ((uint8_t)1)
/* GenDig zone id data. Use KeyID to specify a slot in the Data zone or a transport key in the
 * hardware array.
 */
#define GENDIG_ZONE_DATA         ((uint8_t)2)
/* GenDig zone id shared nonce. KeyID specifies the location of the input value in the message
 * generation.
 */
#define GENDIG_ZONE_SHARED_NONCE ((uint8_t)3)
/* GenDig zone id counter. KeyID specifies the monotonic counter ID to be included in the message
 * generation.
 */
#define GENDIG_ZONE_COUNTER      ((uint8_t)4)
/* GenDig zone id key config. KeyID specifies the slot for which the configuration information is to
...
/* Nonce zero (param2): calculation mode TempKey, use TempKey in calculation and return new TempKey
 * value
 */
