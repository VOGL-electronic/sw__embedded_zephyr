...
static int entropy_ateccx08_get_entropy(const struct device *dev, uint8_t *buffer, uint16_t length)
{
	...
	if (atecc_calib_is_locked_config(config->parent) == false) {
		LOG_ERR("Config not locked, no random data available.");
		return -EPERM;
	}

	if (dev_data->buffer_pos > 0) {
		size_t to_copy = MIN(length, dev_data->buffer_pos);
		...
		buffer += to_copy;
		length -= to_copy;
		dev_data->buffer_pos -= to_copy;
	}

	while (length > 0) {
		size_t to_copy;

		ret = atecc_calib_random(config->parent, dev_data->random_buffer);
		if (ret < 0) {
			return ret;
		}
		...
		buffer += to_copy;
		length -= to_copy;
		dev_data->buffer_pos = sizeof(dev_data->random_buffer) - to_copy;
	}
	return ret;
}
